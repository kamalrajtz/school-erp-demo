...
import { getReceivedLeaveRequestsForRole, LEAVE_REQUESTS_UPDATED_EVENT, LEAVE_STATUSES, parseLeaveDate, statusBadgeColor } from './leaveRequestData'
import { getLeaveReceivedViewPath, getLeaveRoutes, resolveRoleKey } from './leaveRequestConfigs'

export default function ReceivedLeaveRequestsList({ roleKey: roleKeyProp, description }) {
    ...
    const requests = useMemo(() => {
        void refreshKey
        return getReceivedLeaveRequestsForRole(roleKey)
    ...
    const filteredRequests = useMemo(() => {
        ...
        return requests.filter((request) => {
            ...
            const haystack = `${request.leaveRequestId} ${request.requestedBy} ${request.role} ${request.leaveType} ${request.status}`.toLowerCase()
            return haystack.includes(query)
        })
    }, [requests, search, statusFilter, fromDate])
    ...
    return (
        <section>
            <div className='bg-white rounded-2xl shadow-md p-4'>
                {description ? <p className='text-sm text-[#667085] mb-4'>{description}</p> : null}
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
                    <div className='flex flex-col gap-y-2'>
                        <label className='text-base font-medium text-[#808080]'>Search</label>
                        <input type='text' value={search} onChange={(event) => setSearch(event.target.value)} placeholder='Leave ID, name...' className='text-sm text-[#808080] border border-[#D9D9D9] rounded-md px-2 py-2 w-full' />
                    </div>
                    <div className='flex flex-col gap-y-2'>
                    ...
                    <div className='flex flex-col gap-y-2'>
                        <button type='button' onClick={clearFilters} className='bg-[#515DEF] text-white uppercase text-sm px-6 py-2 border border-[#515DEF] rounded-lg hover:opacity-90 transition-all duration-200 cursor-pointer mt-auto'>Clear Filters</button>
                    </div>
                </div>
            </div>
                    ...
                    <table className='w-full text-sm text-left'>
                        ...
                        <tbody>
                            {filteredRequests.length === 0 ? (
                                <tr>
                                    <td colSpan={10} className='px-2 py-8 text-center text-[#667085]'>No received leave requests found.</td>
                                </tr>
                            ) : (
                                    ...
                                    <tr key={request.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7]'>
                                        ...
                                        <td className='px-2 py-4 text-center rounded-e-lg'>
                                            <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                                <NavLink to={getLeaveReceivedViewPath(roleKey, request.id)} className='block w-full text-left p-2 hover:bg-[#515DEF] hover:text-white rounded cursor-pointer'>View</NavLink>
                                            </Dropdown>
                                        </td>
                                    </tr>
                                ))
