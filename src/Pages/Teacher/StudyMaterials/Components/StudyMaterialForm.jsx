...
const StudyMaterialForm = ({ record }) => {
    const [uploadDate, setUploadDate] = useState(() => parseDate(record?.uploadDate))
    ...
    return (
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6 lg:mt-8 mt-2'>
            ...
            <div className='flex flex-col gap-y-2 sm:col-span-2 lg:col-span-3'>
                <label htmlFor='material-title' className='text-base font-medium text-[#1E1E1E]'>Material Title:</label>
                <input type='text' id='material-title' defaultValue={record?.materialTitle ?? ''} placeholder='e.g. Real Numbers — Chapter Notes' className={inputClass} />
            </div>
            <div className='flex flex-col gap-y-2 sm:col-span-2 lg:col-span-3'>
                <label htmlFor='description' className='text-base font-medium text-[#1E1E1E]'>Description:</label>
                <textarea id='description' defaultValue={record?.description ?? ''} placeholder='Brief description of the study material...' className={textareaClass} />
            </div>
            <div className='flex flex-col gap-y-2'>
            ...
            <div className='flex flex-col gap-y-2 sm:col-span-2 lg:col-span-3'>
                ...
                >
                    ...
                    <span className='text-sm text-[#808080]'>Choose file (PDF, DOC, PPT, images)</span>
                    <input
                        type='file'
                        id='upload-document'
                        className='hidden'
