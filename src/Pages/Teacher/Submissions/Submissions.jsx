...
import { SUBMISSIONS_LIST, CLASSES, SECTIONS } from './submissionsData'

const CountCell = ({ count, colorClass, statusType, record, onView }) => {
    ...
    return (
        <div className='flex items-center gap-2'>
            ...
            <button
                ...
                className={`inline-flex items-center justify-center size-8 rounded-lg border transition-all duration-200 shrink-0 ${
                    canView
                        ? 'border-[#515DEF33] bg-[#515DEF0D] text-[#515DEF] hover:bg-[#515DEF] hover:text-white hover:border-[#515DEF] cursor-pointer'
                        : 'border-[#E2E8F0] bg-[#F9FAFB] text-[#C4C4C4] cursor-not-allowed'
                }`}
            >
                <Eye size={15} />
            </button>
...
const Submissions = () => {
    ...
    const openStudentsModal = (record, statusType) => {
        setStudentsModal({ record, statusType })
    }
        ...
        <section>
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    <h2 className='text-xl font-medium text-black'>Submissions List</h2>
                    <button
                        type='button'
                    ...
                    <table className='w-full text-sm text-left rtl:text-right'>
                        <thead className='text-xs bg-[#EDEEF5] whitespace-nowrap rounded-lg'>
                            <tr className='rounded-lg'>
                                ...
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Submitted Students</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Pending Students</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase rounded-e-lg'>Late Submissions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {SUBMISSIONS_LIST.map((record) => (
                                <tr key={record.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7] rounded-lg'>
                                    ...
                                    <td className='px-2 py-4'>
                                        <CountCell
                                            count={record.submittedStudents}
                                    ...
                                    <td className='px-2 py-4'>
                                        <CountCell
                                            count={record.pendingStudents}
                                            colorClass={record.pendingStudents > 0 ? 'text-[#FF9800]' : 'text-[#667085]'}
                                            statusType='pending'
                                            record={record}
                                            onView={openStudentsModal}
                                        />
                                    ...
                                    <td className='px-2 py-4 rounded-e-lg'>
                                        <CountCell
                                            count={record.lateSubmissions}
                                            colorClass={record.lateSubmissions > 0 ? 'text-[#F44336]' : 'text-[#667085]'}
                                            statusType='late'
                                            record={record}
                                            onView={openStudentsModal}
                                        />
            ...
            <div className='flex justify-between items-center px-4 mt-4'>
                <p className='text-sm font-medium text-[#515DEF]'>
                    Showing 1 to {SUBMISSIONS_LIST.length} of {SUBMISSIONS_LIST.length} entries
                </p>
