...
const ReceivedLeaveRequests = () => {
    const [requests, setRequests] = useState(() => getAllAdminLeaveRequests())
    const [fromDate, setFromDate] = useState(new Date())
    const [toDate, setToDate] = useState(new Date())
    const [exportModal, setExportModal] = useState(false)

    const refreshRequests = () => setRequests(getAllAdminLeaveRequests())

    return (
        <section>
            <div className='bg-white rounded-2xl shadow-md p-4'>
                <p className='text-sm text-[#667085] mb-4'>Leave requests submitted by Admin for Super Admin approval.</p>
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center mb-4'>
                    <h2 className='text-xl font-medium text-black'>Received Leave Requests (Admin)</h2>
                    <div className='flex gap-x-2'>
                        <button type='button' onClick={refreshRequests} className='bg-white text-[#515DEF] text-sm px-4 py-2 rounded-md border border-[#515DEF] hover:bg-[#515DEF] hover:text-white transition-all duration-200 cursor-pointer'>Refresh</button>
                                ...
                                <tr key={request.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7]'>
                                    ...
                                    <td className='px-2 py-4 text-center rounded-e-lg'>
                                        <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                            <NavLink to={`/super-admin/leave-request/received/view/${request.id}`} className='block w-full text-left p-2 hover:bg-[#515DEF] hover:text-white rounded cursor-pointer'>View</NavLink>
                                        </Dropdown>
                                    </td>
                                </tr>
                            ))}
