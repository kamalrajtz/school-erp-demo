...
const AddLeaveRequest = () => {
    ...
    const handleSubmit = () => {
        if (!leaveType || !reason.trim()) return
        ...
        navigate(`/gate-keeper/leave-request/my-requests/view/${created.id}`)
    }
    ...
    return (
        <section>
            <div className='bg-white rounded-2xl shadow-md p-4'>
                ...
                <p className='text-sm text-[#667085] mt-2'>This request will be sent to {REQUESTED_TO} for approval.</p>
                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:mt-8 mt-2'>
                    <div className='flex flex-col gap-y-2'>
                        <label className='text-base font-medium text-[#1E1E1E]'>Employee ID:</label>
                        <input type='text' readOnly value={DEFAULT_GATEKEEPER.employeeId} className={`${inputClass} bg-[#F9FAFB]`} />
                    </div>
                    <div className='flex flex-col gap-y-2'>
                        <label className='text-base font-medium text-[#1E1E1E]'>Gate Keeper Name:</label>
                        <input type='text' readOnly value={DEFAULT_GATEKEEPER.name} className={`${inputClass} bg-[#F9FAFB]`} />
                    </div>
                    <div className='flex flex-col gap-y-2'>
                        <label className='text-base font-medium text-[#1E1E1E]'>Requested To:</label>
                        <input type='text' readOnly value={REQUESTED_TO} className={`${inputClass} bg-[#F9FAFB]`} />
                    </div>
                    <div className='flex flex-col gap-y-2'>
                    ...
                    <div className='flex flex-col gap-y-2 lg:col-span-3'>
                        ...
                        <textarea rows={3} value={reason} onChange={(e) => setReason(e.target.value)} className={inputClass} placeholder='Describe the reason for leave...' />
                    </div>
                </div>
            </div>

            <div className='flex sm:justify-end justify-center gap-x-4 mt-6'>
                <button type='button' onClick={() => navigate('/gate-keeper/leave-request/my-requests')} className='bg-white text-[#515DEF] text-sm px-12 py-2 rounded-md border border-[#515DEF] hover:bg-[#515DEF] hover:text-white transition-all duration-200 cursor-pointer md:w-auto w-full'>Discard Changes</button>
