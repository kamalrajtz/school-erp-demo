...
import { getLeaveRequestById, statusBadgeColor } from '../../../Common/GateKeeperLeaveRequest/gateKeeperLeaveRequestData'
...
const ViewMyLeaveRequest = () => {
    ...
    return (
        <section className='space-y-6'>
            <button type='button' onClick={() => navigate('/gate-keeper/leave-request/my-requests')} className='inline-flex items-center gap-2 text-sm text-[#515DEF] border border-[#515DEF] rounded-md px-4 py-2 hover:bg-[#515DEF] hover:text-white transition-colors cursor-pointer'>
                <ArrowLeft size={18} />
                Back to list
            </button>
                ...
                <>
                    <div className='bg-white rounded-2xl shadow-md p-4'>
                        ...
                        <p className='text-sm text-[#667085] mt-2'>
                            {request.leaveType} · {request.fromDate} – {request.toDate} · Requested to {request.requestedTo}
                        </p>
                    </div>
                    ...
                    <Section title='Leave Request Details'>
                        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6'>
                            ...
                            {request.managerRemarks ? <div className='lg:col-span-3'><Field label='Manager Remarks' value={request.managerRemarks} /></div> : null}
                        </div>
                    </Section>

                    {request.status === 'Pending' ? (
                        <div className='bg-[#FF980008] border border-[#FF980033] rounded-2xl p-4 text-sm text-[#667085]'>
                            Your leave request is pending approval from Gate Keeper Manager.
                        </div>
                    ) : null}
                </>
            )}
...
export default ViewMyLeaveRequest
