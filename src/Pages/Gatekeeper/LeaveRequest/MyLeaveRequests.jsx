...
const MyLeaveRequests = () => {
    ...
    useEffect(() => {
        setRequests(getLeaveRequestsByEmployeeId(DEFAULT_GATEKEEPER.employeeId))
    }, [location.key])

    return (
        <section>
            <div className='bg-white rounded-2xl shadow-md p-4 mb-6'>
                <h1 className='text-2xl font-semibold text-black'>Leave Request</h1>
                <p className='text-sm text-[#667085] mt-1'>
                    Apply for leave and track approval status from Gate Keeper Manager.
                </p>
            </div>
            ...
            <div className='bg-white rounded-2xl shadow-md p-4'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:mt-8 mt-2'>
                    <div className='flex flex-col gap-y-2'>
                        ...
                        <input type='text' placeholder='Leave ID, type...' className='text-sm text-[#808080] border border-[#D9D9D9] rounded-md px-2 py-2 w-full' />
                    </div>
                    <div className='flex flex-col gap-y-2'>
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    ...
                    <div className='flex gap-x-2'>
                        <NavLink to='/gate-keeper/leave-request/my-requests/add' className='bg-[#515DEF] text-white text-sm px-4 py-2 rounded-md hover:opacity-90 transition-all duration-200 cursor-pointer flex items-center gap-x-2'>
                            <Plus size={16} />
                            Apply Leave
                        </NavLink>
                            ...
                            {requests.length === 0 ? (
                                <tr>
                                    <td colSpan={9} className='px-2 py-8 text-center text-[#667085]'>
                                        No leave requests yet. Click &quot;Apply Leave&quot; to submit a request to Gate Keeper Manager.
                                    </td>
                                </tr>
                            ) : (
                                    ...
                                    <tr key={request.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7]'>
                                        ...
                                        <td className='px-2 py-4 text-center rounded-e-lg'>
                                            <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                                <NavLink to={`/gate-keeper/leave-request/my-requests/view/${request.id}`} className='block w-full text-left p-2 hover:bg-[#515DEF] hover:text-white rounded cursor-pointer'>View</NavLink>
                                            </Dropdown>
                                        </td>
                                    </tr>
                                ))
