...
const RootCauseAnalysis = () => {
    ...
    const [records, setRecords] = useState(() => getRcaRecords())
    const [fromDate, setFromDate] = useState(new Date())
    const [toDate, setToDate] = useState(new Date())
    const [exportModal, setExportModal] = useState(false)

    useEffect(() => {
        setRecords(getRcaRecords())
    }, [location.pathname])
        ...
        <section>
            <div className='bg-white rounded-2xl shadow-md p-4'>
                <p className='text-sm text-[#667085] mb-4'>
                    Review root cause analyses submitted by assigned employees for audit observations.
                </p>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:mt-8 mt-2'>
                    <div className='flex flex-col gap-y-2'>
                        ...
                        <input type='text' id='search' placeholder='Observation ID, audit, title...' className='text-sm font-normal text-[#808080] border border-[#D9D9D9] rounded-md px-2 py-2 w-full' />
                    </div>
                    <div className='flex flex-col gap-y-2'>
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    <h2 className='text-xl font-medium text-black'>Root Cause Analysis List</h2>
                    <button
                        type='button'
                ...
                <div className='relative overflow-x-auto'>
                    <table className='w-full text-sm text-left rtl:text-right min-w-[1200px]'>
                        <thead className='text-xs bg-[#EDEEF5] whitespace-nowrap rounded-lg'>
                            <tr>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase rounded-s-lg'>Observation ID</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Audit Number</th>
                                ...
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Observation Title</th>
                                ...
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Report To</th>
                                ...
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Submitted On</th>
                        ...
                        <tbody>
                            {records.map((record) => (
                                <tr key={record.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7]'>
                                    <td className='px-2 py-4 font-medium text-[#515DEF] rounded-s-lg whitespace-nowrap'>{record.observationId}</td>
                                    <td className='px-2 py-4 whitespace-nowrap'>{record.auditNumber}</td>
                                    <td className='px-2 py-4 max-w-[140px] truncate' title={record.auditName}>{record.auditName}</td>
                                    <td className='px-2 py-4 whitespace-nowrap'>{record.department}</td>
                                    <td className='px-2 py-4 max-w-[120px] truncate' title={record.section}>{record.section}</td>
                                    <td className='px-2 py-4 font-medium text-[#1E1E1E] max-w-[160px] truncate' title={record.observationTitle}>{record.observationTitle}</td>
                                    ...
                                    <td className='px-2 py-4 whitespace-nowrap'>{record.reportTo}</td>
                                    <td className='px-2 py-4 whitespace-nowrap'>{record.dueDate}</td>
                                    <td className='px-2 py-4 whitespace-nowrap'>{record.submittedBy}</td>
                                    <td className='px-2 py-4 whitespace-nowrap'>
                                        {record.submittedOn}
                                        <span className='block text-xs text-[#808080]'>{record.submittedOnTime}</span>
                                    </td>
                                    <td className='px-2 py-4 text-center rounded-e-lg'>
                                        <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                            <NavLink
                                                to={`/process-auditor/corrective-actions/root-cause-analysis/view/${record.id}`}
