...
import { getMyLeaveRequests, LEAVE_STATUSES, statusBadgeColor } from './leaveRequestData'
...
const MyLeaveRequests = () => {
    ...
    useEffect(() => {
        setRequests(getMyLeaveRequests())
    }, [location.key])
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    ...
                    <div className='flex gap-x-2'>
                        <NavLink to='/gatekeeper-manager/leave-request/my-requests/add' className='bg-[#515DEF] text-white text-sm px-4 py-2 rounded-md hover:opacity-90 transition-all duration-200 cursor-pointer flex items-center gap-x-2'>
                            <Plus size={16} />
                            Apply Leave
                        </NavLink>
                                ...
                                <tr key={request.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7]'>
                                    ...
                                    <td className='px-2 py-4 text-center rounded-e-lg'>
                                        <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                            <NavLink to={`/gatekeeper-manager/leave-request/my-requests/view/${request.id}`} className='block w-full text-left p-2 hover:bg-[#515DEF] hover:text-white rounded cursor-pointer'>View</NavLink>
                                        </Dropdown>
                                    </td>
                                </tr>
                            ))}
