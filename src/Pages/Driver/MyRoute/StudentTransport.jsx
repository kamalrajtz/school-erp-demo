...
import {
    STUDENT_TRANSPORTS,
    TRANSPORT_STATUSES,
    filterStudentTransports,
} from './studentTransportData'
...
const StudentTransport = () => {
    ...
    const filteredRecords = useMemo(() => filterStudentTransports({
        records: STUDENT_TRANSPORTS,
        search,
        status,
        fromDate,
        toDate,
        ...
        <section className='space-y-6'>
            <div className='bg-white rounded-2xl shadow-md p-4'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:mt-8 mt-2'>
                    <div className='flex flex-col gap-y-2'>
                        <label htmlFor='student-search' className='text-base font-medium text-[#808080]'>Search</label>
                        <input
                            id='student-search'
                            type='text'
                            value={search}
                            onChange={(event) => setSearch(event.target.value)}
                            placeholder='Student ID, name, route...'
            ...
            <div className='bg-white rounded-2xl shadow-md p-4'>
                <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4'>
                    <h2 className='text-xl font-semibold text-black'>Student Transport List</h2>
                    <button
                        type='button'
                ...
                <div className='relative overflow-x-auto'>
                    <table className='w-full text-sm text-left min-w-[1200px]'>
                        <thead className='text-xs bg-[#EDEEF5] whitespace-nowrap rounded-lg'>
                            <tr>
                                <th className={`${thClass} rounded-s-lg`}>Student ID</th>
                                <th className={thClass}>Student Name</th>
                                <th className={thClass}>Class & Section</th>
                                <th className={thClass}>Admission Number</th>
                                <th className={thClass}>Route Name</th>
                                <th className={thClass}>Start Location</th>
                                <th className={thClass}>End Location</th>
                                <th className={thClass}>Parent Contact</th>
                                <th className={`${thClass} rounded-e-lg`}>Transport Pass</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filteredRecords.length === 0 ? (
                                <tr>
                                    <td colSpan={9} className='px-2 py-8 text-center text-[#667085]'>
                                        No student transport records found for the selected filters.
                                    </td>
                                </tr>
                            ) : (
                                filteredRecords.map((record) => (
                                    <tr key={record.id} className='border-b border-[#f2f4f7] hover:bg-[#f2f4f7]'>
                                        ...
                                        <td className={tdClass}>{record.admissionNumber}</td>
                                        <td className={tdClass}>{record.routeName}</td>
                                        <td className={tdClass}>{record.startLocation}</td>
                                        <td className={tdClass}>{record.endLocation}</td>
                                        <td className={tdClass}>{record.parentContact}</td>
                                        <td className={`${tdClass} rounded-e-lg`}>{record.transportPass}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
