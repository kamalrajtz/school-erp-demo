        ...
        <section>
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    <h2 className='text-xl font-medium text-black'>Enquiry List</h2>
                    <div className='flex gap-x-2'>
                        <NavLink to="/front-office/add-enquiry" className='bg-[#515DEF] text-white text-sm px-4 py-2 rounded-md hover:opacity-90 transition-all duration-200 cursor-pointer flex items-center gap-x-2'>
                            <Plus size={16} />
                            Add Enquiry
                        </NavLink>
                    ...
                    <table className="w-full text-sm text-left rtl:text-right">
                        <thead className="text-xs bg-[#EDEEF5] whitespace-nowrap rounded-lg">
                            <tr className='rounded-lg'>
                                ...
                                <th className="px-2 py-3.5 text-[#0C1E5B] font-medium uppercase">Mode of Meeting</th>
                                <th className="px-2 py-3.5 text-[#0C1E5B] font-medium uppercase">Reason</th>
                                <th className="px-2 py-3.5 text-[#0C1E5B] font-medium uppercase">Scheduled Date</th>
                                ...
                                <th className="px-2 py-3.5 text-[#0C1E5B] font-medium uppercase">Pincode</th>
                        ...
                        <tbody>
                            {enquiries.map((row, index) => (
                                <tr key={index} className="border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7] rounded-lg">
                                    <td className="px-2 py-4 rounded-s-lg">{row.name}</td>
                                    <td className="px-2 py-4">{row.mobile}</td>
                                    <td className="px-2 py-4">{row.mode}</td>
                                    <td className="px-2 py-4">{row.reason}</td>
                                    <td className="px-2 py-4">{row.scheduledDate}</td>
                                    <td className="px-2 py-4 max-w-[180px] truncate">
                                        {row.meetingLink.startsWith('http') ? (
                                            <a href={row.meetingLink} target="_blank" rel="noreferrer" className="text-[#515DEF] hover:underline">
                                                {row.meetingLink}
                                            </a>
                                        ) : (
                                            row.meetingLink
                                    ...
                                    <td className="px-2 py-4">{row.state}</td>
                                    <td className="px-2 py-4">{row.city}</td>
                                    <td className="px-2 py-4">{row.pincode}</td>
