...
import {
    STATUS_OPTIONS,
    deleteMaterialGatePass,
    formatDisplayTime,
    getMaterialGatePasses,
    statusBadgeColor,
...
const MaterialGatePassList = () => {
    const [records, setRecords] = useState(() => getMaterialGatePasses())
    ...
    const filteredRecords = useMemo(() => {
        return records.filter((record) => {
            ...
            const matchesSearch =
                !query ||
                record.mgpNo.toLowerCase().includes(query) ||
                record.driverName.toLowerCase().includes(query) ||
                record.vehicleNo.toLowerCase().includes(query) ||
    ...
    const handleDelete = () => {
        ...
        deleteMaterialGatePass(selectedId)
        setRecords(getMaterialGatePasses())
        ...
        <section>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:mt-8 mt-2'>
                    <div className='flex flex-col gap-y-2'>
                        ...
                        <input
                            ...
                            placeholder='MGP No., driver, vehicle...'
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    <h2 className='text-xl font-medium text-black'>Material Gate Pass List</h2>
                    <div className='flex gap-x-2'>
                        <NavLink
                            to='/front-office/add-material-gate-pass'
                            className='bg-[#515DEF] text-white text-sm px-4 py-2 rounded-md hover:opacity-90 transition-all duration-200 cursor-pointer flex items-center gap-x-2'
                        >
                            <Plus size={16} />
                            Add Material Gate Pass
                        </NavLink>
                        <button
                            type='button'
                    ...
                    <table className='w-full text-sm text-left'>
                        <thead className='text-xs bg-[#EDEEF5] whitespace-nowrap rounded-lg'>
                            <tr>
                                ...
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Vehicle No.</th>
                            ...
                            {paginatedRecords.length === 0 ? (
                                ...
                                paginatedRecords.map((record) => (
                                    <tr key={record.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7]'>
                                        ...
                                        <td className='px-2 py-4'>{record.driverName}</td>
                                        <td className='px-2 py-4'>{record.vehicleNo}</td>
                                        <td className='px-2 py-4'>
                                            <span className={`px-2 py-1 rounded-lg text-xs font-semibold whitespace-nowrap ${statusBadgeColor[record.status] ?? statusBadgeColor.Pending}`}>
                                                {record.status}
                                            </span>
                                        </td>
                                        ...
                                        <td className='px-2 py-4 text-center rounded-e-lg'>
                                            <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                                <NavLink to={`/front-office/material-gate-pass/view/${record.id}`} className='block w-full text-left p-2 hover:bg-[#515DEF] hover:text-white rounded cursor-pointer'>View</NavLink>
                                                <NavLink to={`/front-office/material-gate-pass/edit/${record.id}`} className='block w-full text-left p-2 hover:bg-[#515DEF] hover:text-white rounded cursor-pointer'>Edit</NavLink>
                                                <button
                                                    type='button'
                                                    onClick={() => {
                                                ...
                                                <NavLink to={`/front-office/material-gate-pass/view/${record.id}?print=1`} className='block w-full text-left p-2 hover:bg-[#515DEF] hover:text-white rounded cursor-pointer'>
                                                    <span className='inline-flex items-center gap-1'><Printer size={14} /> Print</span>
                                                </NavLink>
                                            </Dropdown>
                                        </td>
                                    </tr>
            ...
            <div className='flex justify-between items-center px-4 mt-4'>
                ...
                <div className='flex gap-x-2'>
                    <button
                        ...
                        className='size-8 flex justify-center items-center p-2 bg-white text-[#515DEF] border border-[#E2E8F0] hover:bg-[#515DEF] hover:text-white rounded-full cursor-pointer disabled:opacity-50'
                    >
                        <ChevronLeft size={16} />
                    </button>
                    {Array.from({ length: totalPages }, (_, i) => i + 1).slice(0, 5).map((page) => (
                        <button
                            ...
                            className={`size-8 flex justify-center items-center p-2 border border-[#E2E8F0] rounded-full cursor-pointer ${currentPage === page ? 'bg-[#515DEF] text-white' : 'bg-white text-[#515DEF] hover:bg-[#515DEF] hover:text-white'}`}
                        >
                            {page}
                        </button>
                    ))}
            ...
            <ExportModal
                ...
                exportDescription={
                    <>
                        You are exporting {filteredRecords.length} record{filteredRecords.length === 1 ? '' : 's'}{' '}
                        {statusFilter && <span className='text-[#515DEF]'>( Filtered: {statusFilter} )</span>}
                    </>
                }
            />
            ...
            {deleteModal && (
                <div className='fixed inset-0 z-500 flex items-center justify-center'>
                    ...
                    <div className='relative z-10 w-full max-w-md rounded-xl bg-white shadow-lg p-6'>
                        <h3 className='text-lg font-semibold text-black'>Delete Material Gate Pass?</h3>
