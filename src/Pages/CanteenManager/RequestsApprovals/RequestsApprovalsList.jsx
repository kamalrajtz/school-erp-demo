            ...
            <div className='bg-white rounded-2xl shadow-md p-4'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:mt-8 mt-2'>
                    <div className='flex flex-col gap-y-2'>
                        ...
                        <input type="text" placeholder="Request ID, title..." className='text-sm text-[#808080] border border-[#D9D9D9] rounded-md px-2 py-2 w-full' />
                    </div>
                    <div className='flex flex-col gap-y-2'>
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    ...
                    <div className='flex gap-x-2'>
                        <NavLink to="/canteen-manager/requests-approvals/add-request" className='bg-[#515DEF] text-white text-sm px-4 py-2 rounded-md hover:opacity-90 transition-all duration-200 cursor-pointer flex items-center gap-x-2'>
                            <Plus size={16} />
                            Add Request
                        </NavLink>
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="text-xs bg-[#EDEEF5] whitespace-nowrap rounded-lg">
                            <tr>
                                ...
                                <th className="px-2 py-3.5 text-[#0C1E5B] font-medium uppercase">Sent To</th>
                                <th className="px-2 py-3.5 text-[#0C1E5B] font-medium uppercase">Status</th>
                                <th className="px-2 py-3.5 text-[#0C1E5B] font-medium uppercase">Approval Date</th>
                        ...
                        <tbody>
                            {REQUESTS.map((request) => (
                                <tr key={request.requestId} className="border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7]">
                                    ...
                                    <td className="px-2 py-4 max-w-[180px] truncate" title={request.title}>{request.title}</td>
                                    <td className="px-2 py-4">{request.requestedAmount}</td>
                                    <td className="px-2 py-4">
                                        <span className={`px-2 py-1 rounded-lg text-xs font-semibold whitespace-nowrap ${priorityBadgeColor[request.priority]}`}>
                                            {request.priority}
                                        </span>
                                    </td>
                                    <td className="px-2 py-4">{request.submittedDate}</td>
                                    <td className="px-2 py-4">{request.sentTo}</td>
                                    <td className="px-2 py-4">
                                        <span className={`px-2 py-1 rounded-lg text-xs font-semibold whitespace-nowrap ${statusBadgeColor[request.status]}`}>
                                            {request.status}
                                        </span>
                                    </td>
                                    <td className="px-2 py-4">{request.approvalDate}</td>
                                    <td className="px-2 py-4 text-center rounded-e-lg">
                                        <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                            <NavLink to={`/canteen-manager/requests-approvals/view-request/${request.requestId}`} className="w-full text-left p-2 hover:bg-[#515DEF] hover:text-white rounded cursor-pointer block">View</NavLink>
