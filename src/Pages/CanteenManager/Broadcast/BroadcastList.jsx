...
import { BROADCASTS, priorityBadgeColor, statusBadgeColor } from './broadcastData'
        ...
        <section>
            <div className='bg-white rounded-2xl shadow-md p-4'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:mt-8 mt-2'>
                    <div className='flex flex-col gap-y-2'>
                        ...
                        <input type="text" placeholder="Broadcast ID, title..." className='text-sm text-[#808080] border border-[#D9D9D9] rounded-md px-2 py-2 w-full' />
                    </div>
                    <div className='flex flex-col gap-y-2'>
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    <h2 className='text-xl font-medium text-black'>Broadcast</h2>
                    <div className='flex gap-x-2'>
                        <NavLink to="/canteen-manager/broadcast/add-broadcast" className='bg-[#515DEF] text-white text-sm px-4 py-2 rounded-md hover:opacity-90 transition-all duration-200 cursor-pointer flex items-center gap-x-2'>
                            <Plus size={16} />
                            Add Broadcast
                        </NavLink>
                ...
                <div className="relative overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        <thead className="text-xs bg-[#EDEEF5] whitespace-nowrap rounded-lg">
                            <tr>
                                <th className="px-2 py-3.5 text-[#0C1E5B] font-medium uppercase rounded-s-lg">Broadcast ID</th>
                                <th className="px-2 py-3.5 text-[#0C1E5B] font-medium uppercase">Title</th>
                                <th className="px-2 py-3.5 text-[#0C1E5B] font-medium uppercase">Audience</th>
                                ...
                                <th className="px-2 py-3.5 text-[#0C1E5B] font-medium uppercase">Publish Date</th>
                                <th className="px-2 py-3.5 text-[#0C1E5B] font-medium uppercase">Expiry Date</th>
                        ...
                        <tbody>
                            {BROADCASTS.map((broadcast) => (
                                <tr key={broadcast.broadcastId} className="border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7]">
                                    <td className="px-2 py-4 font-medium text-[#1E1E1E] rounded-s-lg">{broadcast.broadcastId}</td>
                                    <td className="px-2 py-4 max-w-[160px] truncate" title={broadcast.title}>{broadcast.title}</td>
                                    <td className="px-2 py-4">{broadcast.audience}</td>
                                    <td className="px-2 py-4">{broadcast.category}</td>
                                    <td className="px-2 py-4">
                                        <span className={`px-2 py-1 rounded-lg text-xs font-semibold whitespace-nowrap ${priorityBadgeColor[broadcast.priority]}`}>
                                            {broadcast.priority}
                                        </span>
                                    </td>
                                    <td className="px-2 py-4">{broadcast.sentBy}</td>
                                    <td className="px-2 py-4 whitespace-nowrap">{broadcast.publishDate}</td>
                                    <td className="px-2 py-4 whitespace-nowrap">{broadcast.expiryDate}</td>
                                    <td className="px-2 py-4">
                                        <span className={`px-2 py-1 rounded-lg text-xs font-semibold whitespace-nowrap ${statusBadgeColor[broadcast.status]}`}>
                                            {broadcast.status}
                                        </span>
                                    </td>
                                    <td className="px-2 py-4 text-center rounded-e-lg">
                                        <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                            <NavLink to={`/canteen-manager/broadcast/view-broadcast/${broadcast.broadcastId}`} className="w-full text-left p-2 hover:bg-[#515DEF] hover:text-white rounded cursor-pointer block">View</NavLink>
            ...
            <div className='flex justify-between items-center px-4 mt-4'>
                <p className='text-sm font-medium text-[#515DEF]'>Showing 1 to {BROADCASTS.length} of {BROADCASTS.length} entries</p>
