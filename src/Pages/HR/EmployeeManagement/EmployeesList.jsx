        ...
        <section>
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    <h2 className='text-xl font-medium text-black'>Employees List</h2>
                    <button
                        type='button'
                        ...
                        <tbody>
                            {EMPLOYEES.map((employee) => (
                                <tr key={employee.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7]'>
                                    <td className='px-2 py-4 font-medium text-[#1E1E1E] rounded-s-lg'>{employee.id}</td>
                                    <td className='px-2 py-4 whitespace-nowrap'>{employee.name}</td>
                                    <td className='px-2 py-4'>{employee.department}</td>
                                    <td className='px-2 py-4 max-w-[180px] truncate' title={employee.designation}>{employee.designation}</td>
                                    <td className='px-2 py-4'>
                                        <span className={`px-2 py-1 rounded-lg text-xs font-semibold whitespace-nowrap ${employeeStatusBadgeColor[employee.status]}`}>
                                            {employee.status}
                                        </span>
                                    </td>
                                    <td className='px-2 py-4 whitespace-nowrap'>{employee.joiningDate}</td>
                                    <td className='px-2 py-4 text-center rounded-e-lg'>
                                        <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                            <NavLink
                                                to={`/hr/employee-management/employee-profile/${employee.id}`}
            ...
            <div className='flex justify-between items-center px-4 mt-4'>
                <p className='text-sm font-medium text-[#515DEF]'>Showing 1 to {EMPLOYEES.length} of {EMPLOYEES.length} entries</p>
