...
import { CATEGORY_OPTIONS, MOCK_BROADCASTS } from './broadcastData'
        ...
        <section>
            <div className='bg-white rounded-2xl shadow-md p-4'>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:mt-8 mt-2'>
                    ...
                    <div className='flex flex-col gap-y-2'>
                        <label htmlFor='category' className='text-base font-medium text-[#808080]'>Broadcast Category</label>
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    <h2 className='text-xl font-medium text-black'>Broadcast List</h2>
                    <div className='flex gap-x-2'>
                        <NavLink to='/joint-director-assistant/broadcast/add-broadcast' className='bg-[#515DEF] text-white text-sm px-4 py-2 rounded-md hover:opacity-90 transition-all duration-200 cursor-pointer flex items-center gap-x-2'>
                            <Plus size={16} />
                            Add Broadcast
                        </NavLink>
                        ...
                        <tbody>
                            {MOCK_BROADCASTS.map((item) => (
                                <tr key={item.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7] rounded-lg'>
                                    ...
                                    <td className='px-2 py-4'>{item.broadcastDate}</td>
                                    <td className='px-2 py-4 text-center rounded-e-lg'>
                                        <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                            <NavLink to={`/joint-director-assistant/broadcast/view-broadcast/${item.id}`} className='block w-full text-left p-2 hover:bg-[#515DEF] hover:text-white rounded cursor-pointer'>
                                                View
                                            </NavLink>
            ...
            <div className='flex justify-between items-center px-4 mt-4'>
                <p className='text-sm font-medium text-[#515DEF]'>Showing 1 to {MOCK_BROADCASTS.length} of {MOCK_BROADCASTS.length} entries</p>
