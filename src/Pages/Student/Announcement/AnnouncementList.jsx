...
import { CATEGORY_OPTIONS, getAnnouncementsForPortal } from './announcementData'

const AnnouncementList = () => {
    const { routePrefix, portalMode } = useActiveStudent()
    const [fromDate, setFromDate] = useState(new Date())
    const [toDate, setToDate] = useState(new Date())
    const [exportModal, setExportModal] = useState(false)

    const announcements = useMemo(
        () => getAnnouncementsForPortal(portalMode),
        [portalMode],
    )
        ...
        <section>
            <div className='bg-white rounded-2xl shadow-md p-4 mb-6'>
                ...
                <p className='text-sm text-[#667085] mt-1'>
                    School announcements published for students and parents.
                </p>
            </div>
            ...
            <div className='bg-white rounded-2xl shadow-md p-4'>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:mt-8 mt-2'>
                    <div className='flex flex-col gap-y-2'>
                        ...
                        <input type='text' id='search' placeholder='Title, ID...' className='text-sm font-normal text-[#808080] border border-[#D9D9D9] rounded-md px-2 py-2 w-full' />
                    </div>
                    <div className='flex flex-col gap-y-2'>
                        <label htmlFor='category' className='text-base font-medium text-[#808080]'>Category</label>
                                ...
                                <tr key={item.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7]'>
                                    ...
                                    <td className='px-2 py-4 text-center rounded-e-lg'>
                                        <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                            <NavLink to={`${routePrefix}/announcement/view/${item.id}`} className='block w-full text-left p-2 hover:bg-[#515DEF] hover:text-white rounded cursor-pointer'>View</NavLink>
                                        </Dropdown>
                                    </td>
                                </tr>
                            ))}
            ...
            <div className='flex justify-between items-center px-4 mt-4'>
                <p className='text-sm font-medium text-[#515DEF]'>Showing 1 to {announcements.length} of {announcements.length} entries</p>
