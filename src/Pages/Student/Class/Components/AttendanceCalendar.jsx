...
// ─── Types ────────────────────────────────────────────────────────────────────
// AttendanceStatus: "P" | "A" | "H" | null
// AttendanceData: Record<string, AttendanceStatus>   e.g. { "2025-05-01": "P" }
//
// fetchAttendance(year: number, month: number): Promise<AttendanceData>
...
async function defaultFetchAttendance(year, month) {
  ...
  // Swap this block with a real API call:
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (weekday === 0 || weekday === 6) {
      data[key] = "H"; // Weekends are always holidays
    } else {
      // Roughly 1 in 6 weekdays absent, rest present
      ...
      <div className="relative flex flex-col items-center gap-1.5">
        ...
        <span
          className={`
            ...
            ${!isCurrentMonth ? "text-slate-300" : isToday ? "text-[#1358CF]" : "text-slate-700"}
          `}
        >
          {day}
        </span>

        {/* Status badge — only for current month */}
...
export default function AttendanceCalendar({
  ...
  const [viewMonth, setViewMonth] = useState(initialMonth ?? now.getMonth()); // 0-indexed
  ...
  // Build grid cells (prev tail + current + next lead)
  ...
  return (
    <div className="bg-white rounded-2xl shadow-md p-7 w-full select-none">
      ...
      <div className="flex items-center justify-center gap-5 mb-7">
        <button
          onClick={prevMonth}
          className="w-9 h-9 flex items-center justify-center rounded-full text-slate-700
                     hover:bg-slate-100 active:scale-90 transition-all cursor-pointer"
...
//   return res.json();
//   // → { "2025-05-01": "P", "2025-05-03": "H", "2025-05-07": "A", ... }
// }
//
// <AttendanceCalendar fetchAttendance={fetchAttendance} />
//
// Props:
//   fetchAttendance(year, month) → Promise<Record<"YYYY-MM-DD", "P"|"A"|"H">>
//   initialYear?  number   (default: 2025)
//   initialMonth? number   (0-indexed, default: 4 = May)
