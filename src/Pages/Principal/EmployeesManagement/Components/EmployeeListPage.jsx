...
const EmployeeListPage = ({ title, columns, rows, viewPath }) => {
    ...
    return (
        <section>
                    ...
                    <table className="w-full text-sm text-left rtl:text-right">
                        <thead className="text-xs bg-[#EDEEF5] whitespace-nowrap rounded-lg">
                            <tr className='rounded-lg'>
                                {columns.map((col, index) => (
                                    <th
                                        key={col.key}
                                        className={`px-2 py-3.5 text-[#0C1E5B] font-medium uppercase ${index === 0 ? 'rounded-s-lg' : ''} ${index === columns.length ? 'rounded-e-lg' : ''}`}
                                    >
                                        {col.label}
                                    </th>
                                ))}
                        ...
                        <tbody>
                            {rows.map((row, rowIndex) => (
                                <tr key={rowIndex} className="border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7] rounded-lg">
                                    {columns.map((col, colIndex) => (
                                        <td key={col.key} className={`px-2 py-4 ${colIndex === 0 ? 'rounded-s-lg' : ''}`}>
                                            {col.isProfile ? (
                                    ...
                                    <td className="px-2 py-4 text-center rounded-e-lg">
                                        <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                            <NavLink to={viewPath} className="w-full text-left p-2 hover:bg-[#515DEF] hover:text-white rounded cursor-pointer">
                                                View
                                            </NavLink>
                                        </Dropdown>
                                    </td>
            ...
            <div className='flex justify-between items-center px-4 mt-4'>
                <p className='text-sm font-medium text-[#515DEF]'>Showing 1 to {rows.length} of {rows.length} entries</p>
