...
import { VEHICLES, VEHICLE_TYPES } from './vehicleManagementData'
        ...
        <section>
            <div className='bg-white rounded-2xl shadow-md p-4'>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:mt-8 mt-2'>
                    <div className='flex flex-col gap-y-2'>
                        ...
                        <input type='text' id='search' placeholder='Vehicle ID, number, driver...' className='text-sm font-normal text-[#808080] border border-[#D9D9D9] rounded-md px-2 py-2 w-full' />
                    </div>
                    <div className='flex flex-col gap-y-2'>
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    <h2 className='text-xl font-medium text-black'>Vehicle Management</h2>
                    <div className='flex gap-x-2'>
                        <NavLink
                            to='/transport-manager/vehicle-management/add-vehicle'
                    ...
                    <table className='w-full text-sm text-left rtl:text-right'>
                        <thead className='text-xs bg-[#EDEEF5] whitespace-nowrap rounded-lg'>
                            <tr className='rounded-lg'>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase rounded-s-lg'>Vehicle ID</th>
                                ...
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Capacity</th>
                        ...
                        <tbody>
                            {VEHICLES.map((vehicle) => (
                                <tr key={vehicle.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7] rounded-lg'>
                                    <td className='px-2 py-4 font-medium text-[#1E1E1E] rounded-s-lg'>{vehicle.id}</td>
                                    <td className='px-2 py-4'>{vehicle.vehicleNumber}</td>
                                    <td className='px-2 py-4'>{vehicle.vehicleType}</td>
                                    <td className='px-2 py-4'>{vehicle.capacity}</td>
                                    <td className='px-2 py-4'>{vehicle.lastServiceDate}</td>
                                    <td className='px-2 py-4'>{vehicle.driverName}</td>
                                    <td className='px-2 py-4 text-center rounded-e-lg'>
                                        <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                            <NavLink
                                                to={`/transport-manager/vehicle-management/view/${vehicle.id}`}
                                                className='block w-full text-left p-2 hover:bg-[#515DEF] hover:text-white rounded cursor-pointer'
                                            >
                                                View
                                            </NavLink>
                                            <NavLink
                                                // to={`/transport-manager/vehicle-management/edit/${vehicle.id}`}
            ...
            <div className='flex justify-between items-center px-4 mt-4'>
                <p className='text-sm font-medium text-[#515DEF]'>Showing 1 to {VEHICLES.length} of {VEHICLES.length} entries</p>
