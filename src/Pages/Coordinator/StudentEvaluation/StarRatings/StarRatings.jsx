...
import { getStarRatings, getRatingValue } from './starRatingsData'

const StarRatings = () => {
    const location = useLocation()
    const [ratings, setRatings] = useState(() => getStarRatings())
        ...
        <section>
            <div className='bg-white rounded-2xl shadow-md p-4'>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:mt-8 mt-2'>
                    <div className='flex flex-col gap-y-2'>
                        ...
                        <input type='text' id='search' placeholder='Name, roll number...' className='text-sm font-normal text-[#808080] border border-[#D9D9D9] rounded-md px-2 py-2 w-full' />
                    </div>
                    <div className='flex flex-col gap-y-2'>
                        <label htmlFor='month-filter' className='text-base font-medium text-[#808080]'>Month</label>
                        <input type='text' id='month-filter' placeholder='e.g. May 2026' className='text-sm font-normal text-[#808080] border border-[#D9D9D9] rounded-md px-2 py-2 w-full' />
                    </div>
                    <div className='flex flex-col gap-y-2'>
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    ...
                    <div className='flex gap-x-2'>
                        <NavLink
                            to='/coordinator/student-evaluation/star-ratings/add'
                    ...
                    <table className='w-full text-sm text-left rtl:text-right'>
                        <thead className='text-xs bg-[#EDEEF5] whitespace-nowrap rounded-lg'>
                            <tr className='rounded-lg'>
                                ...
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Roll Number</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Class & Section</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Attendance Rating</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Academic Rating</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Activity Rating</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Discipline Rating</th>
                        ...
                        <tbody>
                            {ratings.map((entry) => (
                                <tr key={entry.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7] rounded-lg'>
                                    <td className='px-2 py-4 font-medium text-[#1E1E1E] rounded-s-lg'>{entry.ratingId}</td>
                                    ...
                                    <td className='px-2 py-4'>{entry.className} - {entry.section}</td>
                                    <td className='px-2 py-4'>
                                        <StarRatingDisplay rating={getRatingValue(entry, 'attendance')} />
                                    </td>
                                    <td className='px-2 py-4'>
                                        <StarRatingDisplay rating={getRatingValue(entry, 'academic')} />
                                    </td>
                                    <td className='px-2 py-4'>
                                        <StarRatingDisplay rating={getRatingValue(entry, 'activity')} />
                                    </td>
                                    <td className='px-2 py-4'>
                                        <StarRatingDisplay rating={getRatingValue(entry, 'discipline')} />
                                    </td>
                                    <td className='px-2 py-4 font-medium text-[#0C1E5B]'>{entry.overallPoints} Points</td>
                                    <td className='px-2 py-4 max-w-[180px] truncate' title={entry.description}>{entry.description}</td>
                                    <td className='px-2 py-4 text-center rounded-e-lg'>
                                        <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                            <NavLink
                                                to={`/coordinator/student-evaluation/star-ratings/view/${entry.id}`}
            ...
            <div className='flex justify-between items-center px-4 mt-4'>
                <p className='text-sm font-medium text-[#515DEF]'>
                    Showing 1 to {ratings.length} of {ratings.length} entries
                </p>
