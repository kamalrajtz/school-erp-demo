...
import { getQuestionBanks, EXAM_TYPES, SUBJECTS, CLASSES, SECTIONS } from './questionBanksData'

const QuestionBanks = () => {
    const location = useLocation()
    const [records, setRecords] = useState(() => getQuestionBanks())
        ...
        <section>
            <div className='bg-white rounded-2xl shadow-md p-4'>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:mt-8 mt-2'>
                    <div className='flex flex-col gap-y-2'>
                        ...
                        <input type='text' id='search' placeholder='Document ID, paper title...' className='text-sm font-normal text-[#808080] border border-[#D9D9D9] rounded-md px-2 py-2 w-full' />
                    </div>
                    <div className='flex flex-col gap-y-2'>
                    ...
                    <div className='flex flex-col gap-y-2'>
                        <label htmlFor='exam-type-filter' className='text-base font-medium text-[#808080]'>Exam Type</label>
                        <select id='exam-type-filter' className='text-sm font-normal text-[#808080] border border-[#D9D9D9] rounded-md px-2 py-2 w-full'>
                            <option value=''>All</option>
                            {EXAM_TYPES.map((type) => (
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    <h2 className='text-xl font-medium text-black'>Question Banks List</h2>
                    <div className='flex gap-x-2'>
                        <NavLink
                            to='/coordinator/student-evaluation/questions-banks/add'
                    ...
                    <table className='w-full text-sm text-left rtl:text-right'>
                        <thead className='text-xs bg-[#EDEEF5] whitespace-nowrap rounded-lg'>
                            <tr className='rounded-lg'>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase rounded-s-lg'>Document ID</th>
                                ...
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Exam Type</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Paper Title</th>
                        ...
                        <tbody>
                            {records.map((record) => (
                                <tr key={record.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7] rounded-lg'>
                                    <td className='px-2 py-4 font-medium text-[#1E1E1E] rounded-s-lg'>{record.documentId}</td>
                                    <td className='px-2 py-4 font-medium text-[#1E1E1E]'>{record.subject}</td>
                                    <td className='px-2 py-4'>{record.classSection ?? `${record.className} - ${record.section}`}</td>
                                    <td className='px-2 py-4'>{record.examType}</td>
                                    <td className='px-2 py-4 font-medium text-[#1E1E1E]'>{record.paperTitle}</td>
                                    <td className='px-2 py-4'>
                                        {record.fileName && record.fileName !== '—' ? (
                                            <span className='flex items-center gap-x-2'>
                                                <img src={pdf_icon} alt='file' className='w-6 h-6 shrink-0' />
                                                <span className='text-sm text-[#515DEF] max-w-[140px] truncate' title={record.fileName}>
                                                    {record.fileName}
                                                </span>
                                            </span>
                                        ) : (
                                    ...
                                    <td className='px-2 py-4 max-w-[180px] truncate' title={record.description}>{record.description}</td>
                                    <td className='px-2 py-4'>{record.uploadDate}</td>
                                    <td className='px-2 py-4 text-center rounded-e-lg'>
                                        <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                            <NavLink
                                                to={`/coordinator/student-evaluation/questions-banks/view/${record.id}`}
