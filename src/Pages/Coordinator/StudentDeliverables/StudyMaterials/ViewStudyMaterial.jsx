...
import { DEFAULT_ROUTE_BASE, getStudyMaterialById } from './studyMaterialsData'
...
const ViewStudyMaterial = ({ routeBase = DEFAULT_ROUTE_BASE }) => {
    const { id } = useParams()
    const navigate = useNavigate()
    const record = getStudyMaterialById(id)
        ...
        <section className='space-y-6'>
            ...
            {!record ? (
                <div className='bg-white rounded-2xl shadow-md p-8 text-center text-[#667085]'>
                    Study material record not found or could not be loaded.
                </div>
            ) : (
                <>
                    <div className='bg-white rounded-2xl shadow-md p-4'>
                        <h1 className='text-2xl font-semibold text-black'>{record.materialId}</h1>
                    ...
                    <Section title='Study Material Information'>
                        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6'>
                            <Field label='Material ID' value={record.materialId} />
                    ...
                    <Section title='Details'>
                        <StudyMaterialForm record={record} readOnly />
                    </Section>
                </>
            )}
        </section>
