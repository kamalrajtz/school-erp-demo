...
import { ATTENDANCE_HISTORY, CLASSES, SECTIONS } from './attendanceHistoryData'
        ...
        <section>
            <div className='bg-white rounded-2xl shadow-md p-4'>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:mt-8 mt-2'>
                    <div className='flex flex-col gap-y-2'>
                        ...
                        <input type='text' id='search' placeholder='Student ID, name...' className='text-sm font-normal text-[#808080] border border-[#D9D9D9] rounded-md px-2 py-2 w-full' />
                    </div>
                    <div className='flex flex-col gap-y-2'>
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    <h2 className='text-xl font-medium text-black'>Attendance History List</h2>
                    <button
                        type='button'
                    ...
                    <table className='w-full text-sm text-left rtl:text-right'>
                        <thead className='text-xs bg-[#EDEEF5] whitespace-nowrap rounded-lg'>
                            <tr className='rounded-lg'>
                                ...
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Working Days</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Present (Days)</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Absent (Days)</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Late (Days)</th>
                        ...
                        <tbody>
                            {ATTENDANCE_HISTORY.map((record) => (
                                <tr key={record.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7] rounded-lg'>
                                    <td className='px-2 py-4 font-medium text-[#1E1E1E] rounded-s-lg'>{record.studentId}</td>
                                    <td className='px-2 py-4 font-medium text-[#1E1E1E]'>{record.studentName}</td>
                                    <td className='px-2 py-4'>{record.workingDays}</td>
                                    <td className='px-2 py-4'>{record.present}</td>
                                    <td className='px-2 py-4'>{record.absent}</td>
                                    <td className='px-2 py-4'>{record.late}</td>
                                    <td className='px-2 py-4 font-medium text-[#1E1E1E] rounded-e-lg'>{record.percentage}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            <div className='flex justify-between items-center px-4 mt-4'>
                <p className='text-sm font-medium text-[#515DEF]'>
                    Showing 1 to {ATTENDANCE_HISTORY.length} of {ATTENDANCE_HISTORY.length} entries
                </p>
