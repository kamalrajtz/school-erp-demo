...
import { getMarksEntries, CLASSES, SECTIONS, SUBJECTS, EXAM_NAMES } from './enterMarksData'

const EnterMarks = () => {
    const location = useLocation()
    const [records, setRecords] = useState(() => getMarksEntries())
    const [exportModal, setExportModal] = useState(false)
    const [editRequestModal, setEditRequestModal] = useState(false)
    const [deleteRequestModal, setDeleteRequestModal] = useState(false)

    useEffect(() => {
        setRecords(getMarksEntries())
    }, [location.pathname])
        ...
        <section>
            <div className='bg-white rounded-2xl shadow-md p-4'>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:mt-8 mt-2'>
                    <div className='flex flex-col gap-y-2'>
                        ...
                        <input type='text' id='search' placeholder='Entry ID, student name...' className='text-sm font-normal text-[#808080] border border-[#D9D9D9] rounded-md px-2 py-2 w-full' />
                    </div>
                    <div className='flex flex-col gap-y-2'>
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    <h2 className='text-xl font-medium text-black'>Marks Entry List</h2>
                    <div className='flex gap-x-2'>
                        <NavLink
                            to='/coordinator/marks-results/enter-marks/add'
                        ...
                        <tbody>
                            {records.map((record) => (
                                <tr key={record.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7] rounded-lg'>
                                    ...
                                    <td className='px-2 py-4 font-medium text-[#1E1E1E]'>{record.obtainedMarks}</td>
                                    <td className='px-2 py-4'>
                                        <span className='px-2 py-1 rounded-lg text-xs font-semibold bg-[#515DEF33] text-[#515DEF]'>
                                            {record.grade}
                                        </span>
                                    </td>
                                    <td className='px-2 py-4 max-w-[180px] truncate' title={record.remarks}>{record.remarks}</td>
                                    <td className='px-2 py-4 text-center rounded-e-lg'>
                                        <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                            <NavLink
                                                to={`/coordinator/marks-results/enter-marks/view/${record.id}`}
