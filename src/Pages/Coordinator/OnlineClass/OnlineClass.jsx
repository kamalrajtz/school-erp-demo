...
import {
    ...
    CLASSES,
    SECTIONS,
    ONLINE_CLASS_STATUSES,
    statusBadgeColor,
} from './onlineClassData'
        ...
        <section>
            <div className='bg-white rounded-2xl shadow-md p-4'>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:mt-8 mt-2'>
                    <div className='flex flex-col gap-y-2'>
                        ...
                        <input type='text' id='search' placeholder='Online class ID, topic...' className='text-sm font-normal text-[#808080] border border-[#D9D9D9] rounded-md px-2 py-2 w-full' />
                    </div>
                    <div className='flex flex-col gap-y-2'>
                    ...
                    <div className='flex flex-col gap-y-2'>
                        ...
                        <select id='status-filter' className='text-sm font-normal text-[#808080] border border-[#D9D9D9] rounded-md px-2 py-2 w-full'>
                            <option value=''>All</option>
                            {ONLINE_CLASS_STATUSES.map((status) => (
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    ...
                    <div className='flex gap-x-2'>
                        <NavLink
                            to='/coordinator/class/online-class/add'
                    ...
                    <table className='w-full text-sm text-left rtl:text-right'>
                        <thead className='text-xs bg-[#EDEEF5] whitespace-nowrap rounded-lg'>
                            <tr className='rounded-lg'>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase rounded-s-lg'>Online Class ID</th>
                                ...
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Meeting Platform</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Meeting Link</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Status</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase rounded-e-lg'>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {ONLINE_CLASSES.map((record) => (
                                <tr key={record.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7] rounded-lg'>
                                    <td className='px-2 py-4 font-medium text-[#1E1E1E] rounded-s-lg'>{record.onlineClassId}</td>
                                    ...
                                    <td className='px-2 py-4 max-w-xs'>{record.topic}</td>
                                    <td className='px-2 py-4'>{record.date}</td>
                                    <td className='px-2 py-4'>{record.startTime}</td>
                                    <td className='px-2 py-4'>{record.endTime}</td>
                                    <td className='px-2 py-4'>{record.meetingPlatform}</td>
                                    <td className='px-2 py-4 max-w-xs truncate' title={record.meetingLink}>{record.meetingLink}</td>
                                    ...
                                    <td className='px-2 py-4 text-center rounded-e-lg'>
                                        <Dropdown buttonContent={<EllipsisIcon size={16} className='text-black' />}>
                                            <NavLink
                                                to={`/coordinator/class/online-class/view/${record.id}`}
            ...
            <div className='flex justify-between items-center px-4 mt-4'>
                <p className='text-sm font-medium text-[#515DEF]'>
                    Showing 1 to {ONLINE_CLASSES.length} of {ONLINE_CLASSES.length} entries
                </p>
