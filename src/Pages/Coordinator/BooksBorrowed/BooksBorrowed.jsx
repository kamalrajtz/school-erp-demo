...
import { BORROWED_BOOKS_LIST, BOOK_STATUSES, statusBadgeColor } from './booksBorrowedData'
        ...
        <section>
            <div className='bg-white rounded-2xl shadow-md p-4'>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:mt-8 mt-2'>
                    <div className='flex flex-col gap-y-2'>
                        ...
                        <input type='text' id='search' placeholder='Book ID, name, author...' className='text-sm font-normal text-[#808080] border border-[#D9D9D9] rounded-md px-2 py-2 w-full' />
                    </div>
                    <div className='flex flex-col gap-y-2'>
            ...
            <div className='bg-white rounded-2xl shadow-md p-4 mt-8'>
                <div className='flex justify-between items-center sm:flex-row flex-col gap-y-2 mb-4'>
                    <h2 className='text-xl font-medium text-black'>Borrowed Books List</h2>
                    <button
                        type='button'
                    ...
                    <table className='w-full text-sm text-left rtl:text-right'>
                        <thead className='text-xs bg-[#EDEEF5] whitespace-nowrap rounded-lg'>
                            <tr className='rounded-lg'>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase rounded-s-lg'>Book ID</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Book Name</th>
                                <th className='px-2 py-3.5 text-[#0C1E5B] font-medium uppercase'>Author Name</th>
                        ...
                        <tbody>
                            {BORROWED_BOOKS_LIST.map((record) => (
                                <tr key={record.id} className='border-b text-[#667085] border-[#f2f4f7] hover:bg-[#f2f4f7] rounded-lg'>
                                    <td className='px-2 py-4 font-medium text-[#1E1E1E] rounded-s-lg'>{record.bookId}</td>
                                    <td className='px-2 py-4 font-medium text-[#1E1E1E]'>{record.bookName}</td>
                                    <td className='px-2 py-4'>{record.authorName}</td>
                                    <td className='px-2 py-4'>{record.issuedDate}</td>
                                    <td className='px-2 py-4'>{record.returnDate}</td>
            ...
            <div className='flex justify-between items-center px-4 mt-4'>
                <p className='text-sm font-medium text-[#515DEF]'>
                    Showing 1 to {BORROWED_BOOKS_LIST.length} of {BORROWED_BOOKS_LIST.length} entries
                </p>
